function [table_out] = Sgp4classifier(inFile, maxInterval)
%% initiate sgp4 dlls
...
for i = 1:numSats
    ...
    [errcode, satID] = calllib('Tle', 'TleGetField', satKeys(i), XF_TLE_SATNUM, satID);
    satEpochDs50UTC = calllib('TimeFunc', 'DTGToUTC', satEpochDTG);
    epochs(i) = satEpochDs50UTC;
    IDs(i) = str2double(satID);
    structure.(strcat('catID', string(IDs(i)))) = table_out;
end
...
%             structure.(strcat('catID', string(IDs(i)))) = ...
%                 [structure.(strcat('catID', string(IDs(i)))); ...
%                 {line1(3:7), ...
%                 pos(1), pos(2), pos(3), ...
%                 vel(1), vel(2), vel(3), ...
