...
#using the 64-bit C-driver and compare the output results (stored in the TestResults folder) 
...
for d in */; do
   ...
   cd $d/Execution
   if [[ "$d" == "Coco" ]]; then
      ./Run_Coco_Test_Cases.sh 64
   fi

   if [[ "$d" == "ElComp" ]]; then
      ./Run_ElComp_Test_Cases.sh 64
   fi

   if [[ "$d" != "Coco" && "$d" != "ElComp" ]]; then
      ./Run_Test_Cases.sh 64
   fi
