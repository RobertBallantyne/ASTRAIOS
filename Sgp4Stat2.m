...
function [table_out] = Sgp4Stat2(applicableTLEs, TLEpath, satEpoch)
...
for i = 1:length(applicableTLEs)
    x = num2str(applicableTLEs(i));
    errCode = calllib('Tle', 'TleLoadFile', [TLEpath '/tle_' x '.inp']);
    if(errCode ~= 0)
       ShowMsgAndTerminate;
    end
end
...
vars = {'catID', 'x', 'y', 'z', 'u', 'v', 'w', 'apo', 'peri', 'a', 'e', 'i', 'raan', 'omega', 'startTime', 'stopTime'};
